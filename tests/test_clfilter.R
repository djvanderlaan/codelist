...
res <- clfilter(codelist, locale = "LOWER")
expect_equal(res$code, c("A", "B", "A.1", "A.2", "B.1"))
expect_equal(res$label, letters[1:5])
expect_equal(res$locale, rep("LOWER", 5))

res <- clfilter(codelist, locale = NA)
expect_equal(res$code, c("A", "B", "A.1", "A.2", "B.1"))
expect_equal(res$label, letters[1:5])
expect_equal(res$locale, rep("LOWER", 5))

expect_warning(res <- clfilter(codelist, locale = "FOOBAR" ))
expect_equal(res$code, c("A", "B", "A.1", "A.2", "B.1"))
expect_equal(res$label, letters[1:5])
expect_equal(res$locale, rep("LOWER", 5))

expect_error(res <- clfilter(codelist, locale = c("UPPER", "LOWER")))
expect_error(res <- clfilter(codelist, locale = character(0)))

res <- clfilter(codelist, level = 1, locale = "UPPER")
...
res <- clfilter(codelist, levels = 1)
expect_equal(res$code, c("A.1", "A.2", "B.1"))
expect_equal(res$parent, NULL)

res <- clfilter(codelist, levels = 2)
expect_equal(res$code, c("A.1.1", "B.1.1"))
expect_equal(res$parent, NULL)


res <- clfilter(codelist, levels = c(1,2))
expect_equal(res$code, c("A.1", "A.2", "A.1.1", "B.1", "B.1.1"))
expect_equal(res$parent, NULL)

res <- clfilter(codelist, levels = c(0,2))
expect_equal(res$code, c("A", "B", "A.1.1", "B.1.1"))
expect_equal(res$parent, NULL)

res <- clfilter(codelist, levels = c(0,1))
expect_equal(res$code, c("A", "B", "A.1", "A.2", "B.1"))
expect_equal(res$parent, c(NA, NA, "A", "A", "B"))

res <- clfilter(codelist, levels = c(0,2), check_levels = FALSE)
expect_equal(res$code, c("A", "B", "A.1.1", "B.1.1"))
expect_equal(res$parent, c(NA, NA, "A.1", "B.1"))

expect_error(res <- clfilter(codelist, levels = integer(0)))
expect_error(res <- clfilter(codelist, levels = c(0:3)))
