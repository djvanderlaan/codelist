...
expect_warning(res <- code(x, codelist))
expect_equal(res, c("B01", "A03", NA))

# This only works if all from different locale
x <- c("Hammer", "Knikkers", NA)
expect_error(res <- code(x, codelist))

# And only when no locale given
x <- c("Hamer", "Knikkers", NA)
expect_error(res <- code(x, codelist, locale = "EN"))
...
# Different locale
x <- c("Hamer", "Knikkers", NA)
res <- code(x, codelist, locale = "NL")
expect_equal(res, c("B01", "A03", NA))

# Invalid locale
x <- c("Hammer", "Marbles", NA)
expect_warning(res <- code(x, codelist, locale = "FOO"))
expect_equal(res, c("B01", "A03", NA))
