...
# invalid values in x
x   <- c("FOO", "X", "A01", NA)
expect_error(res <- labels.coded(x, codelist = codelist))

# invalid codelist
x   <- c("A", "X", "A01", NA)
expect_error(res <- labels.coded(x, codelist = "FOO"))
...
x   <- c("A", "X", "A01", NA)
attr(x, "codelist") <- codelist
res <- labels.coded(x, missing = FALSE, droplevels = TRUE)
l   <- codelist$label[codelist$locale == "EN"]
expect_equal(res, factor(c("Toys", "Unknown object", "Teddy Bear", NA), 
  levels = c("Toys", "Teddy Bear", "Unknown object")))
...
# set locale to NL
x   <- c("A", "X", "A01", NA)
res <- labels.coded(x, codelist = codelist, locale = "NL")
...
x   <- c("A", "X", "A01", NA)
res <- labels.coded(x, codelist = codelist, locale = "EN")
l   <- codelist$label[codelist$code != "X" & codelist$locale == "EN"]
expect_equal(res, factor(c("Toys", NA, "Teddy Bear", NA), levels = l))

# invalid locale
x   <- c("A", "X", "A01", NA)
expect_warning(res <- labels.coded(x, codelist = codelist, locale = "FOO"))
l   <- codelist$label[codelist$code != "X" & codelist$locale == "EN"]
expect_equal(res, factor(c("Toys", NA, "Teddy Bear", NA), levels = l))
...
x   <- c("A", "X", "A01", NA)
res <- labels.coded(x, codelist = objectcodes)
l   <- codelist$label[codelist$code != "X" & codelist$locale == "EN"]
expect_equal(res, factor(c("Toys", NA, "Teddy Bear", NA), levels = l))
...
x   <- c("A", "X", "A01", NA)
res <- labels.coded(x, codelist = codelist2)
